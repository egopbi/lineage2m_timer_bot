...
from models import Base, Timer, BossRespawn, User
...
class DataBaseAPI():
    ...
    async def get_boss_respawn(self, user_id, boss_name) -> int | bool:
        async with self.async_session() as session:
            try:
                ...
                database_logger.error(f"Error while getting boss respawn by user {user_id}: {str(e)}")
                return False
    ...
    async def get_all_boss_respawns(self, user_id) -> list[BossRespawn] | bool:
        async with self.async_session() as session:
            try:
                ...
                database_logger.error(f"Error while getting all boss respawns by user {user_id}: {str(e)}")
                return False
    ...
    async def get_all_chat_timers(self, user_id, chat_id) -> list[Timer] | bool:
        async with self.async_session() as session:
            try:
                await self._delete_expired_timers()
                result = await session.execute(
                    select(Timer)
                ...
                database_logger.error(f"Error while getting all chat timers by user {user_id}: {str(e)}")
                return False
        ...
        async with self.async_session() as session:
            try:
                ...
                if count < total_count:
                    ...
                    timers = await self.get_all_my_timers(user_id, chat_id)
        ...
        async with self.async_session() as session:
            try:
                ...
                if count < total_count:
                    ...
                    timers = await self.get_all_timers(user_id, chat_id)
            ...
            async with session.begin():
                try:
                    ...
                    if timer.user_id != user_id:
                        database_logger.error(
                            f"In chat {chat_id} User {user_id} tried to delete alien timer_id: {timer_id}"
                        )
                        return False
                    ...
                    if timer:
                        ...
                        await session.commit()
                        database_logger.success(f"User {user_id} deleted timer with timer_id: {timer_id}")
                        return True
                    else:
                        database_logger.error(
    ...
    async def _delete_expired_timers(self):
        async with self.async_session() as session:
            try:
                ...
                if expired_timers:
                    ...
                    database_logger.info(f"There is no expired timers")   
